...
## v1.1.9 (2026-08-03) 移动端交互：FAB 不被进度条挡 + 面板/歌词独立开关

- **FAB 被大进度条挡住**: 进度条 `content.css` 为 `position:fixed; z-index:99999`，其 `top:70vh` 会随 Kiwi 顶部/底部工具栏显隐而上下移动，故「一会挡住一会不挡住」。修复：把扩展所有 UI 的 z-index 提到进度条之上（FAB/歌词开关 `100000`、面板 `100001`、歌词浮窗 `100002`），绘制顺序永远在进度条之上。
- **「字幕」按钮改为 on/off 拨杆样式**: 面板打开时按钮高亮并显示「字幕 ●」，关闭时恢复「字幕」，状态一目了然。
- **新增右下「词」按钮 = 歌词浮窗独立开关**: 与面板互不影响。点「词」可单独显示/隐藏歌词浮窗；歌词浮窗的 ✕ 也改为「保持隐藏」（新增 `lyricsHidden` 标记，播放循环不再每 250ms 强制把它显示回来）。载入新字幕时 `lyricsHidden` 复位为 false，浮窗照常出现。
- **关闭面板保留歌词**: 面板 ✕ 只关面板（新增 `closePanel()`），歌词浮窗继续显示；面板底部补一行提示「关闭本面板不影响歌词浮窗；右下『词』可单独开关歌词」。
- **移动端工具栏抖动**: 顶部/底部工具栏显隐导致的相对位移属浏览器行为（用户已知），本版仅保证扩展 UI 不被进度条遮挡；FAB 固定在 `bottom:12px`，与进度条（70vh 一带）不重叠。
